...
[This step is complete](https://github.com/jtyu2/data-mgmt-project/tree/main/data), unless we decide to add any more datasets. We already had a FRED API key from in-class assignments, and secured a U.S. Census key easily. The biggest unexpected step was being sure to add a “wait” period between queries. The FRED API has a query rate limit of 120 per minute, and we wanted to be sure that even if we added another FRED dataset to pull data from, we would be able to smoothly integrate them without errors.
